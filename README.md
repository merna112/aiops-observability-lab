...
## Lab Work 5: Automated Incident Response
...
php artisan aiops:respond --interval=20
```
...
- `SERVICE_DEGRADATION` -> `THROTTLE_TRAFFIC`
- `LOCALIZED_ENDPOINT_FAILURE` -> `RESTART_SERVICE`
...
1. Start detector (if not already running):

```bash
php artisan aiops:detect --interval=20
```

2. Run automation engine in a second terminal:

```bash
php artisan aiops:respond --once
```

3. Show response evidence:
